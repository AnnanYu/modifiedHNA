% Driver for reducing the eady example
...
samples2 = [logspace(.01,4,200)*1j,-logspace(.01,4,200)*1j];
configs = {{'benchmarks\eady.mat',10,1:50,samples1,0.01,1},...
...
%% Apply AAA and then modified HNA
opts.reg = 1e-8;
...
for i = 1:length(deg)
    ...
    es(i) = hankel_err(G,@(z) H((z-1)/(z+1))+Gp(z),err_conf);
